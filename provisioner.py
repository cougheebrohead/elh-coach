...
def provision_customer(
    *,
    ...
    """Stand up a REAL paying coach (not a sales demo).

    Differences from provision_demo:
      - is_demo = false, no demo_password, no expiry
      - owner login uses the customer's real email + a temp password
      - empty client roster (the coach invites their own clients)
      - returns subdomain URL + apex fallback URL
    """
    if not brand_name or len(brand_name.strip()) < 2:
        return {"ok": False, "error": "brand_name is required"}
    ...
    # Slug must satisfy ^[a-z0-9-]{2,40}$
    base = (custom_slug or slugify(brand_name) or "coach").strip("-")
    base = SLUG_RE.sub("-", base.lower())[:40].strip("-")
    if len(base) < 2:
        base = "coach"
    ...
    # Plan limits — keep them reasonable per tier
    max_coaches, max_clients = (1, 25) if plan == "coach" else (
        (5, 200) if plan == "studio" else (100_000, 100_000)
    )
    ...
    row = db.fetch_one(
        """insert into tenants
           ...
           values ($1,$2,$3,$4,$5,$6,$2,'active',$7,$8,
                   false,'wizard-customer',$9,$10,$11)
           returning id""",
        slug, brand_name, plan, primary, accent, logo_url,
        max_coaches, max_clients,
        source_url,
        json.dumps(scraped_brand) if scraped_brand else None,
        sales_owner,
    )
    ...
    owner = db.fetch_one(
        """insert into users (tenant_id, email, password_hash, role, name)
           values ($1,$2,$3,'owner',$4) returning id""",
        tenant_id, owner_email, hash_password(temp_password),
        owner_name or owner_email,
    )
    if owner:
        db.execute(
            """insert into coach_profiles (user_id, tenant_id, timezone)
               values ($1,$2,'America/New_York')""",
            owner["id"], tenant_id,
        )
        # Link tenant.owner_user_id so account settings page works
        db.execute(
    ...
    return {
        ...
        "slug": slug,
        "brand_name": brand_name,
        "subdomain_url": f"https://{slug}.elhcoach.app",
        "apex_url": f"https://elh-coach.onrender.com/?tenant={slug}",
        "owner_email": owner_email,
        "owner_temp_password": temp_password,
        "next_steps": [
            "Text the apex_url + temp password to the coach.",
            "They sign in, change their password in Settings.",
            "Once Cloudflare wildcard SSL clears, share the subdomain_url.",
        ],
    }
